...
<div id="layoutSidenav_content">
        ...
        <div class="row g-4">
          ...
          <div class="col-md-3">
            <div class="card bg-success text-white">
              <div class="card-body">
                <h5>Current Stock Quantity</h5>
                <h2>{{ $stockquantity ?? '0' }}</h2>
    ...
    const totalRevenue = @json($dailyStock); // Revenue for each day
    const totalOrders = @json($dailySold); // Orders for each day

    // Area Chart (Total Revenue)
    const ctxArea = document.getElementById('myAreaChart').getContext('2d');
     const currentStock = new Date().toLocaleString('default', { month: 'long' });
    new Chart(ctxArea, {
        type: 'line',
        data: {
            labels: days, // X-axis: Days of January
            datasets: [{
                 label: `Current Stock of [${currentStock}] month`,
        ...
        options: {
            responsive: true,
            scales: {
                x: { title: { display: true, text: 'Days of Current Month' } },
                y: { title: { display: true, text: 'Curent Stock' }, beginAtZero: true }
            },
            plugins: {
                tooltip: {
    ...
    const soldInvoicetMonth = new Date().toLocaleString('default', { month: 'long' });
new Chart(ctxBar, {
    type: 'bar',
    data: {
        labels: @json($days), // X-axis: Days of January
        datasets: [{
            label: `Sold Stock Invoices of [${soldInvoicetMonth}] month`,
            data: @json($dailySold), // Orders data
    ...
    options: {
        ...
        scales: {
            x: { 
                title: { display: true, text: 'Days of Current Month' } 
            },
            y: { 
                title: { display: true, text: 'Sold Stock Invoices' },
